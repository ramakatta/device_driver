...
int main()
{
	...
	ioctl( fd, MY_GET_SIZE, &size );
	printf("New size of the buffer is %d\n", size);		

	
	/* lets fill the buffer with character '=' */
	c = '=';
	ioctl(fd, MY_FILL_CHAR, &c);

      	/* test if it worked */
	read( fd, my_buf, 70 );
	printf("New contents\n%s\n", my_buf);

	/* lets ecrypt the buffer with character 'p' */
	c = 'p';
	ioctl(fd, MY_ENCRYPT, &c);
      	/* test if it worked */
	read( fd, my_buf, 70 );
	printf("Encrypted contents\n%s\n", my_buf);
	/* lets ecrypt the buffer with character 'p' */
	c = 'p';
	ioctl(fd, MY_DECRYPT, &c);
      	/* test if it worked */
	read( fd, my_buf, 70 );
	printf("Deccrypted contents\n%s\n", my_buf);
